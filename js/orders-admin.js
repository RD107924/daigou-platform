      ...
      orders.forEach((order) => {
          ...
          order.items && Array.isArray(order.items)
            ? order.items
                .map(
                  (item) =>
                    `<li>
                            ${item.title || "未知商品"} (x${item.quantity || 1})
                            <br>
                            <small style="color:#6c757d">備註: ${
                              item.notes || "無"
                            }</small>
                        </li>`
                )
        ...
        const customerInfoHtml = `
                    <ul class="info-list">
                        ...
                        <li><strong>統編:</strong> ${order.taxId || "無"}</li>
                    </ul>
                `;
